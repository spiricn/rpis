...
public class Locator {
    ...
    private static final int NUM_RESPONSE_LISTEN_ATTEMPTS = 2;
    ...
    public ServerInfo findServer() {
        ...
        // List for responses for a while
        int numAttempts = NUM_RESPONSE_LISTEN_ATTEMPTS;

        while(numAttempts-- > 0){
            ServerInfo server = receiveResponse();

            if(server == null){
                continue;
            }

            if(findServer(server.getUid()) == null){
                mServers.add(server);
                return server;
            }
            else{
    ...
    private ServerInfo findServer(String uid){
